...
const createSender = (): { sender: WindowStartRequestSender; started: ReturnType<typeof vi.fn> } => {
...
describe('UsageWindowStartService', () => {
  it('arms on the first observation without sending a request', async () => {
    ...
    await service.observeAsync([window({ usedPercent: 60, resetAt: '2026-01-01T00:00:00Z' })], settings({}))
  ...
  it('sends a minimal request after a usage reset drops below 5%', async () => {
    ...
    await service.observeAsync([window({ usedPercent: 60, resetAt: '2026-01-01T00:00:00Z' })], settings({}))
    await service.observeAsync([window({ usedPercent: 0, resetAt: '2026-01-01T00:00:00Z' })], settings({}))
  ...
  it('warns when the reset timestamp moves later on an unused session', async () => {
    ...
    await service.observeAsync([window({ usedPercent: 0, resetAt: '2026-01-01T00:00:00Z' })], settings({}))
    await service.observeAsync([window({ usedPercent: 0, resetAt: '2026-01-01T01:00:00Z' })], settings({}))
