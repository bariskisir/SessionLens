...
export default class AlibabaProvider extends BaseApiKeyProvider {
  async query(context: ProviderQueryContext): Promise<MetricResult | null> {
    const apiKey = this.resolveApiKey(context)
    if (!apiKey) return null
    const now = context.now

    const document = (await postJsonWithHeaders(ENDPOINT, {}, alibabaHeaders(apiKey))) as unknown
    ...
    const quotas =
      getArray(root, 'codingPlanQuotaWindows') ??
      getArray(root, 'quotaWindows') ??
      getArray(root, 'data')
    if (quotas) {
