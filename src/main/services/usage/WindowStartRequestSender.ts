...
export default class WindowStartRequestSender {
    ...
    switch (request.providerName.toLowerCase()) {
      ...
      default:
        throw new ProviderError(
          `Window starting is not supported for ${request.providerName}.`,
        )
    }
  }
   ...
   * @returns Header key-value map
   */
  private claudeHeaders(accessToken: string, beta: string, jsonContent: boolean): Record<string, string> {
    const headers: Record<string, string> = {
      Accept: 'application/json',
      Authorization: `Bearer ${accessToken}`,
  ...
  private parseClaudeModels(root: Record<string, unknown>): DynamicModel[] {
    ...
    return models
      .map((entry) => {
        ...
        const supportsEffort = effortCapability !== null && getBoolean(effortCapability, 'supported')
        return {
          id,
          displayName: getString(record, 'display_name') ?? id,
          description: '',
  ...
  private selectAntigravityModelForBucket(
    ...
    const bucketFamilies = new Set(this.modelFamilyWords(bucketLabel).map((word) => word.toLowerCase()))
  ...
  private modelFamilyWords(value: string): string[] {
    ...
    const separators = [...new Set(value.split('').filter((character) => !/[a-z0-9]/i.test(character)))]
    const words = value
  ...
  private modelSizeScore(model: DynamicModel): number {
    ...
    if (
      /nano|mini|haiku|lite/i.test(value)
    ) {
      return 1
    }
  ...
  private antigravityUserAgent(cliVersion: string): string {
    ...
    const os = currentPlatform === 'win32' ? 'windows' : currentPlatform === 'darwin' ? 'darwin' : 'linux'
