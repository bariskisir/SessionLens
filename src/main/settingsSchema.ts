...
const normalizeIconLayout = (layout: TrayIconLayoutSettings): TrayIconLayoutSettings => {
  const mode: IconLayoutMode =
    layout.mode === 'manual' ? 'manual' : 'auto'
...
export const normalizeSettingsPatch = (patch: AppSettingsPatch): AppSettingsPatch => {
  ...
  if (patch.models !== undefined) {
    normalized.models = {
      smallModelSelector:
        patch.models.smallModelSelector?.trim() || DEFAULT_SMALL_MODEL_SELECTOR,
    }
  }
 ...
 */
export const parsePersistedSettings = (input: unknown): AppSettings => normalizeSettings(input)
